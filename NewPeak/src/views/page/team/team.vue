<template>
	<div id="app" style="background: #F4F4F4;padding-bottom:1rem">
		<div class="showImg">
		...
		<div class="team">
			<div style="display: flex;margin-top: 20px;">
				<div v-for="(item1, index111) in list" :key="index111" @click="jumpTeam(index111)" style="display: flex;margin-left: 20px; ">
					<div class="btnCon" :class="changeColor == index111 ? 'active' : ''" @click="checkTab(item1.cate_id)">{{ item1.name }}</div>
				</div>
			</div>
		</div>
		<div>
			<div class="people">
				<div
					...
					@click="jump(item)"
				>
					<img :src="getImgUrl(item.original_image)" style="width: 9rem;height: 11rem;" />
		...
		<div>
			<ul class="staff">
				<li v-for="(items, index22) in staffInfo" :key="index22" @click="jump(items)">
		...
		<div style="text-align: center;" @click="addData" v-if="isShow">加载更多</div>
	</div>
</template>
<script>
