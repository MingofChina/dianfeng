<template>
  <div id="Header">
    <el-row :gutter="20">
      ...
      <el-col :span="14">
        <el-row
          style="line-height: 4.125rem; font-size: 18px; font-weight: 400"
        >
        ...
        <el-row
          v-if="subscript || subscript == 0"
          class="navLIST"
          :style="{
            width: subscript1 || subscript1 == 0 ? '44.625rem' : '33.25rem',
        ...
        >
          <el-col :span="subscript == 1 ? 8 : 12">
            <img
              :src="navList[subscript].url"
              style="
                width: 11rem;
                height: 8.25rem;
                margin: 4.625rem 2.025rem 2.875rem 1.75rem;
              "
            />
          </el-col>
          <el-col :span="subscript == 0 || subscript == 2 ? 12 : 4">
            <el-col
              :span="subscript == 0 || subscript == 2 ? 12 : 24"
              v-for="(item, index) in navList[subscript].child_column"
              :key="item.id"
              ><div class="navli1" @click="linkFn(item, index)">
                {{ item.name }}
              </div></el-col
            >
          </el-col>
          <el-col
            :span="12"
            v-if="
              (subscript1 || subscript1 == 0) &&
              navList[subscript].child_column[subscript1].childcontent
            "
            style="background: #f2f2f2; height: 26.5rem"
          >
            <el-col
              :span="12"
              class="navli1"
              ...
              :key="index"
              ><div @click="linkFn1(item)">{{ item.title }}</div></el-col
            >
          </el-col>
        </el-row>
      </el-col>
      <el-col :span="5" style="line-height: 4.125rem"
