<template>
  <div id="Header">
    <el-row :gutter="20">
      ...
      <el-col :span="14">
        ...
        font-size: 18px;font-weight:400">
          <el-col :span="4"
            ...
            <div @mouseenter="ulNavFn(item,index,false)"
                 @click="ulNavFn(item,index,true)">{{item.name}}</div>
            <div v-if="index == isActive"
                 style="width:5.1rem;border-top:2px solid #C8000A;position:absolute;top:4.6rem;height:2px;left:3.2rem"></div>
          </el-col>
        </el-row>
        <el-row v-if="subscript|| subscript==0"
                class="navLIST"
                :style="{width:(subscript1||subscript1==0)?'44.625rem':'33.25rem',marginTop:'1rem',zIndex:9999,marginLeft:marginLeft+'rem',background:'#FFFFFF'}">
          <div style="width:100%;height:100%"
               @mouseleave="levalFn">
            <el-col :span="(subscript == 1)?7:12 ">
              <img v-show="navList[subscript].original_image"
                   :src="baseUrl+navList[subscript].original_image"
                   style="width:11rem;height:8.25rem;margin:4.625rem 2.025rem 2.875rem 1.75rem" />
            </el-col>
            <el-col :span="(subscript==0||subscript==2)?12:4">
              <div>
                <el-col :span="(subscript==0||subscript==2)?12:24"
                        class="divhover"
                        :class="{navList: isnavList == index}"
                        v-for="(item,index) in navList[subscript].child_column"
                        :key="item.id">
                  <div :style="{color:index == isActive1 && $route.path.indexOf(item.url)>-1?'#C8000A':''}"
                       class="navli1"
                       style="text-align: center;line-height: .3rem;padding-top:.3rem"
                       @click="linkFn(item,index)"
                       @mouseenter="linkFn2(item,index)">
                    <p class="phover1"
                       style="height:1rem;display:none"></p>{{item.name}}
                  </div>
                </el-col>
              </div>
            ...
            <el-col :span="13"
                    v-if="(subscript1 || subscript1 == 0) &&navList[subscript].child_column[subscript1].childcontent"
                    style="background:#F2F2F2;height:31.9rem;padding-left:1rem">
              <div>
                <el-col :span="12"
                        class="navli1 divhover"
                        v-for="(item,index) in navList[subscript].child_column[subscript1].childcontent"
                        :key="item.id">
                  <div @click="linkFn1(item)"
                       style="font-size: 0.88rem;">{{item.title}}</div>
                </el-col>
              </div>
            </el-col>
          </div>
      ...
      <el-col :span="3"
              style="line-height:4.125rem">
        <div class="grid-content ">
          <div class="bg-purple"></div>
          <div class="grid-div">
            <div class="grid-img"><img src='../assets/bei/路径 80@2x.png'
                   @click="serachFn()"
                   style="width:100%;background:#ffffff" /></div>
