<template>
  <el-card class="box-card">
    ...
    <el-dialog
      ...
      :visible.sync="areaTrainingResults"
    >
      <areaTrainingResults :area-id="areaId" />
    </el-dialog>
  </el-card>
</template>

<script>
import { getEndLongResult } from '@/api/train.js'
import areaTrainingResults from './areaTrainingResults.vue'
...
export default {
  ...
  methods: {
    ...
    async getTable() {
      ...
      const res = await getEndLongResult(params)
      if (res.code == 200) {
        this.getTime()
    ...
    cellClick({ row, column }) {
      if (column.title === '区域') {
        ...
        this.areaTrainingResults = true
      }
    },
    // 导出
    exportExcel() {
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['区域', '区域管理员', '管理员手机', '参训总人数', '合格人数', '不合格人数', '合格率(%)'] // 表格
